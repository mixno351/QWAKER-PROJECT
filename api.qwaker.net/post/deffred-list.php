...
?>
<?php
	$check_post = mysqli_query($connect, "SELECT * FROM `posts` WHERE `user_id` = '$user2_id' AND `archive` = 0 AND `date_view` > '$timeUSER' ORDER BY date_public DESC");
	...
	while($row = mysqli_fetch_assoc($check_post)) {
		...
		echo json_encode(array(
			...
			"post_deffred_time" => intval($row['date_view']),
