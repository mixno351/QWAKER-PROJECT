...
?>
<?php
	$check_follows = mysqli_query($connect, "SELECT * FROM `follows` WHERE `followed_id` = '$user_id' AND `confirm` = 0 ORDER BY date_follow DESC");
	...
	while($row = mysqli_fetch_assoc($check_follows)) {
		$user_id_follow = intval($row['follower_id']);
