...
<?php
	...
	$check_follows = mysqli_query($connect, "SELECT * FROM `follows` WHERE `followed_id` = '$user_id' AND `confirm` = 1 ORDER BY date_follow DESC");
	...
	while($row = mysqli_fetch_assoc($check_follows)) {
		...
		$check_user = mysqli_query($connect, "SELECT * FROM `users` WHERE `id` = '$follower_id' LIMIT 1");
